...
- MySQL 8.0 client for local development
...
Mcpjungle persists all of its data in a MySQL 8.0 database. You can inspect it with any MySQL client, for example the `mysql` CLI:

```bash
mysql -h 127.0.0.1 -P 3306 -u mcpjungle -p mcpjungle
...
If you run the database through the Docker Compose stack, connect to the `db` service on port `3306` using the credentials configured in your compose file. Make sure the database uses the `utf8mb4` character set (collation `utf8mb4_0900_ai_ci`).
...
2. Database connection problems: verify MySQL access
