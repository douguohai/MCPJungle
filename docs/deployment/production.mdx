...
<Tip>
  For production, deploy a separate managed MySQL 8.0 database and point `DATABASE_URL` at its private endpoint. Configure the database for UTC and `utf8mb4`. See [Configure Mcpjungle's database](/deployment/database) for the required settings.
</Tip>
