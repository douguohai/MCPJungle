...
When teams want shared access to MCPs in an enterprise environment, Mcpjungle should run in **enterprise mode** with a dedicated MySQL 8.0 database
...
3. Point Mcpjungle at a separate MySQL 8.0 instance or cluster.
...
docker run -d \
  ...
  -e DATABASE_URL=mysql://user:password@localhost:3306/mcpjungle \
  -p 8080:8080 \
  ghcr.io/mcpjungle/mcpjungle:latest
```

<Tip>
  For production, deploy a separate managed MySQL 8.0 cluster and point `DATABASE_URL` at it rather than running MySQL in the same container. See [Configure Mcpjungle's database](/deployment/database) for all database options.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Database" icon="database" href="/deployment/database">
    Configure MySQL 8.0 correctly for production use.
  </Card>
