...
description: "Use Docker Compose to run mcpjungle locally or as a shared deployment."
---
...
`docker-compose.yaml` is the fastest path to a working local setup. It starts Mcpjungle in development mode alongside MySQL 8.0 and uses the `latest-stdio` image by default, which includes `npx` and `uvx` for common STDIO server workflows. The database is configured for UTC and `utf8mb4`.
...
<Note>
You can also run the mcpjungle image as a standalone container instead of using the Compose files provided by the repository. `DATABASE_URL` remains required and must point to a reachable MySQL 8.0 database:
...
`docker-compose.prod.yaml` starts Mcpjungle in enterprise mode. It is intended for a shared deployment where access is authenticated, management happens in the Dashboard, and metrics are expected to be available.
...
<Steps>
  ...
  <Step title="Initialize the server">
    After the first boot, run `init-server` from your client machine to create the initial admin user and capture the one-time initial password:

    ```bash
    mcpjungle init-server
    ```

    Use the Dashboard after that to create users, permission groups, and personal device tokens. See the [production deployment guide](/deployment/production) for the full initialization and operating model.
  </Step>
</Steps>
