...
`docker-compose.yaml` is the fastest path to a working local setup. It starts Mcpjungle in development mode alongside MySQL and uses the `latest-stdio` image by default, which includes `npx` and `uvx` for common STDIO server workflows.
...
Keep in mind that Mcpjungle requires a MySQL 8.0 database, so point the container at one using the `DATABASE_URL` environment variable.

Running Mcpjungle as a standalone container is only recommended for testing and proofs of concept.
</Note>
