...
Start with MySQL using individual environment variables:

```bash
MYSQL_HOST=localhost MYSQL_USER=root MYSQL_PASSWORD=secret mcpjungle start
```
...
Start with a MySQL database:

```bash
DATABASE_URL="mysql://user:password@localhost:3306/mcpjungle" mcpjungle start
```
