...
The `start` command reads environment variables for server mode, database configuration, timeouts, and telemetry. `DATABASE_URL` is required and must point to a MySQL 8.0 database configured for UTC and `utf8mb4`. A `.env` file in the working directory is loaded automatically if present.

For the complete list and defaults, see [Environment variables](/reference/environment-variables).
...
```bash
DATABASE_URL=mysql://user:pass@host:3306/db mcpjungle start
```

Start on a custom port:

```bash
DATABASE_URL=mysql://user:pass@host:3306/db mcpjungle start --port 9000
```

Start in enterprise mode:

```bash
DATABASE_URL=mysql://user:pass@host:3306/db mcpjungle start --enterprise
```

Start with MySQL 8.0:
