...
The Mcpjungle CLI reads a YAML configuration file at `~/.mcpjungle.conf` on startup. It stores the URL of the Mcpjungle server for CLI requests. Most users never edit this file by hand.
...
The field is optional. If it is absent the CLI falls back to its default value or prompts you to supply it via a CLI flag.
...
`~/.mcpjungle.conf` is usually created or updated by the CLI when you set or reuse a registry URL. You can also edit the file manually if needed.

For the enterprise bootstrap flow and Dashboard workflow, see [Enterprise operations](/reference/cli-enterprise).
...
Enterprise users, permission groups, and personal device tokens are managed in the Dashboard. There is no JSON config-file workflow for those entities.
