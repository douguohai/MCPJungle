---
title: "Connect GitHub Copilot to mcpjungle"
description: "Add Mcpjungle as an MCP server in GitHub Copilot's mcp.json config and give Copilot Chat access to all your registered tools."
---
...
If your Mcpjungle instance runs in enterprise mode, create a personal device token in the Dashboard for the user or workflow that Copilot should use. Copy that token once, then update `mcp.json` to send it:
...
<Warning>
  If Copilot reports no tools, verify that the device token belongs to a user in the correct permission groups.
</Warning>
