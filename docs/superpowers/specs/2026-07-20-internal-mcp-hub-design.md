...
系统尚未投入使用，因此本次重构不保留旧认证和授权模型的兼容能力。实现时可以删除旧字段、旧接口和旧页面，并重新初始化数据库。数据库统一使用 MySQL 8.0，不继续支持 SQLite 和 PostgreSQL。
...
- **单一数据库基线：** 运行、开发和测试环境统一使用 MySQL 8.0，不维护多数据库兼容分支。
...
- 唯一关系型数据库为 MySQL 8.0。
- 生产、开发和自动化测试使用相同的 MySQL 方言与约束行为。
- 仅保留 MySQL 驱动、连接配置、迁移逻辑和数据库测试。
- 删除 SQLite、PostgreSQL 驱动及对应的环境变量、命令行参数、Compose 服务和文档。
- 数据库连接统一通过 `DATABASE_URL` 或 `MYSQL_*` 环境变量配置。
- 默认字符集使用 `utf8mb4`，排序规则使用 `utf8mb4_0900_ai_ci`。
- 时间统一以 UTC 写入 MySQL，管理端按用户时区展示。
- JSON 配置字段使用 MySQL `JSON` 类型；需要筛选和关联的数据使用普通列或关系表，不把核心授权关系放入 JSON。
- 启动时执行结构迁移；迁移失败时服务拒绝启动，不以部分可用状态继续运行。
...
- 删除 SQLite 和 PostgreSQL 支持，只保留 MySQL 8.0。
- 删除 `SQLITE_DB_PATH`、`POSTGRES_*` 及相关命令行参数和文档。
...
- 数据库集成测试统一连接由 Docker Compose 启动的 MySQL 8.0，不使用 SQLite 内存数据库替代。
- 前端构建、类型检查和关键页面交互必须通过。

## 20. 实施顺序

1. 将数据库收敛为 MySQL 8.0，并重建账户、会话、权限组和设备令牌模型。
...
6. 建立调用事件、日聚合和审计日志。
7. 完成工具集合重命名和权限过滤。
8. 更新 MySQL Compose、环境变量、文档和完整测试。
